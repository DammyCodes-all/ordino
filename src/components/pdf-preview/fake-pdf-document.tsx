      ...
      return (
        <View style={{ marginBottom: 10 }}>
          {node.items.map((item, index) => (
            <Text key={`${node.id}-${item}`} style={styles.listItem}>
...
function chunkByPageBreaks(nodes: DocumentNode[]): DocumentNode[][] {
...
export function FakePdfDocument({ document }: { document: DocumentState }) {
...
export async function renderFakePdfBlob(
  ...
  return pdf(<FakePdfDocument document={document} />).toBlob();
}

export function downloadFileName(title: string): string {
  return `${title.replace(/[^\w-]+/g, "_") || "ordino"}.pdf`;
}
