...
export function ChatPanel() {
  ...
  }, [messages, turn.running, stageLabel, mainView]);
      ...
      <header className="flex h-14 shrink-0 items-center gap-3 border-b border-border-subtle px-4 sm:px-6">
        <div className="flex min-w-0 flex-1 items-center gap-3">
          ...
          <div className="flex rounded-xl bg-surface p-1">
            {MAIN_TABS.map((tab) => {
              const disabled = tab.id === "preview" && !publishedPreview;
              return (
                <button
                  key={tab.id}
                  type="button"
                  ...
                  className={`rounded-lg px-3 py-1.5 text-xs transition-colors disabled:cursor-not-allowed disabled:opacity-35 ${
                    mainView === tab.id
        ...
        {!showChat && publishedPreview ? (
          <button
            type="button"
            disabled={turn.running}
            onClick={exportPdf}
            className="rounded-full border border-accent-dim bg-accent-soft px-3 py-1.5 text-xs text-accent transition-colors hover:border-accent disabled:opacity-35"
          >
            Export
          </button>
        ) : null}
            ...
            {empty ? (
              <div className="mx-auto flex min-h-[calc(100%-2rem)] max-w-2xl flex-col items-center justify-center gap-4 py-16 text-center animate-fade-up">
                ...
                <p className="max-w-md text-sm leading-relaxed text-muted">
                  Describe the brief in chat. When a turn finishes, switch to
                  Preview to review the published PDF page.
                </p>
