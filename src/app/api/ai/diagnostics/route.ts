...
export async function POST(request: Request): Promise<Response> {
  ...
  try {
    ...
    return Response.json(
      { code: "INVALID_REQUEST", message: "Unknown action" },
      { status: 400 },
    );
  } catch (error: any) {
    ...
    if (msg.includes("401") || msg.includes("403")) {
      return Response.json(
        { code: "MODEL_AUTH_FAILED", message: msg },
        { status: 401 },
      );
    }
    if (msg.includes("429")) {
      return Response.json(
        { code: "MODEL_RATE_LIMITED", message: msg },
        { status: 429 },
      );
    }
    return Response.json(
      { code: "MODEL_SERVICE_UNAVAILABLE", message: msg },
      { status: 503 },
    );
  }
}
